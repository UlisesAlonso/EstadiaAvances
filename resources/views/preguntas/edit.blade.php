        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Modificar Pregunta o Cuestionario</h2>
                        <p class="text-gray-600">Ajusta la redacción, tipo de pregunta o configuración de asignación</p>
                    </div>
                ...
                <!-- Formulario -->
                <form method="POST" action="{{ route(Auth::user()->isAdmin() ? 'admin.preguntas.update' : 'medico.preguntas.update', $pregunta->id_pregunta) }}" class="space-y-6" id="preguntaForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="descripcion" 
                                      ...
                                      required>{{ old('descripcion', $pregunta->descripcion) }}</textarea>
                            @error('descripcion')
                        ...
                        <div>
                            ...
                            <select name="especialidad_medica" 
                                ...
                                @foreach($especialidades as $esp)
                                    <option value="{{ $esp }}" {{ old('especialidad_medica', $pregunta->especialidad_medica) == $esp ? 'selected' : '' }}>
                                        {{ $esp }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="md:col-span-2" id="opcionesContainer" style="display: {{ $pregunta->tipo === 'opcion_multiple' ? 'block' : 'none' }};">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Opciones de Respuesta * (Mínimo 2 opciones)
                            </label>
                            <div id="opcionesList">
                                @if($pregunta->tipo === 'opcion_multiple' && $pregunta->opciones_multiple)
                                    @foreach($pregunta->opciones_multiple as $index => $opcion)
                                        <div class="flex items-center space-x-2 mb-2">
                                            <input type="text" 
                                                   name="opciones_multiple[]" 
                                                   value="{{ old('opciones_multiple.' . $index, $opcion) }}"
                                                   ...
                                                   required>
                                            <button type="button" onclick="removeOpcion(this)" class="px-3 py-2 bg-red-500 text-white rounded-md hover:bg-red-600" style="display: {{ count($pregunta->opciones_multiple) > 2 ? 'block' : 'none' }};">
                        ...
                        <div>
                            ...
                            <input type="date" 
                                   ...
                                   id="fecha_asignacion"
                                   value="{{ old('fecha_asignacion', $pregunta->fecha_asignacion->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <select name="id_diagnostico" 
                                ...
                                @foreach($diagnosticos as $diagnostico)
                                    <option value="{{ $diagnostico->id_diagnostico }}" {{ old('id_diagnostico', $pregunta->id_diagnostico) == $diagnostico->id_diagnostico ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="id_tratamiento" 
                                ...
                                @foreach($tratamientos as $tratamiento)
                                    <option value="{{ $tratamiento->id_tratamiento }}" {{ old('id_tratamiento', $pregunta->id_tratamiento) == $tratamiento->id_tratamiento ? 'selected' : '' }}>
