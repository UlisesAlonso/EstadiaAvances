...
@section('title', 'Foro de Experiencias')
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="flex justify-between items-center mb-6">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900">Foro de Experiencias</h2>
                        <p class="text-gray-600">Comparte y lee experiencias de otros pacientes</p>
                    </div>
                    <div class="flex space-x-3">
                        @auth
                            @if(auth()->user()->isPaciente())
                                ...
                                <a href="{{ route('paciente.foro.mis-publicaciones') }}" 
                            ...
                            @if(auth()->user()->isAdmin())
                                <a href="{{ route('admin.foro.moderacion') }}" 
                                   class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700">
                                    Moderación
                                </a>
                            @endif
                        @endauth
                        <a href="{{ auth()->user()->isPaciente() ? route('paciente.dashboard') : route('admin.dashboard') }}" 
                           class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                            <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @else
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm font-medium text-green-600">Mis Publicaciones</p>
                                    <p class="text-2xl font-bold text-green-900">{{ $stats['mis_publicaciones'] ?? 0 }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                ...
                <div class="mb-6 bg-gray-50 p-4 rounded-lg">
                    <form method="GET" action="{{ route('foro.index') }}" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="orden" 
                                    ...
                                    <option value="comentarios" {{ request('orden') == 'comentarios' ? 'selected' : '' }}>Más Comentarios</option>
                                </select>
                            </div>
                        </div>
                                        ...
                                        <div class="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                                            ...
                                            @if($publicacion->etiquetas)
                                                <div class="flex items-center">
                                                    ...
                                                    {{ Str::limit($publicacion->etiquetas, 50) }}
                                                </div>
                                            @endif
                                        </div>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500">
                            ...
                            @else
                                No hay publicaciones que mostrar con los filtros seleccionados.
                            @endif
                        </p>
                        @if(auth()->user()->isPaciente())
                            <div class="mt-6">
